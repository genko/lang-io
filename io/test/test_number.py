...
def test_modulo_int_float():
    inp = '5 % 3.2'
    res, space = interpret(inp)
    assert res.number_value == 1.7999999999999998

def test_module_float_float():
    inp = '5.3 % 3.2'
    res, space = interpret(inp)
    assert res.number_value == 2.0999999999999996
...
def test_modulo_float_float():
    inp = '8.3 % 3.2'
    res, space = interpret(inp)
    assert res.number_value == 1.9000000000000004
...
def test_pow():
    ...
    assert res1.number_value >= 0.0289911865471078 \
        and res1.number_value <= 0.0289911865471079 \
        and res2.number_value == res1.number_value
...
def test_divide():
    inp = '3/2'
    res, _ = interpret(inp)
    assert res.number_value == 1.5

def test_division_by_zero():
    inp = '3/0'
    res, _ = interpret(inp)
    assert isinf(res.number_value)

def test_division_zero_by_zero():
    inp = '0/0'
    res, _ = interpret(inp)
    assert isnan(res.number_value)
