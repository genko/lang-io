...
def test_lex_slash_star_multiline_comment_is_ignored():
    inp = """a
    /*
    foo bar
    */
    ...
    assert tokens[0].name != 'Comment'
    assert tokens[1].name != 'Comment'
    assert tokens[2].name != 'Comment'
    assert tokens[3].name != 'Comment'
    assert len(tokens) == 4
def test_lex_slash_star_multiline_comment_is_ignored2():
    inp = 'a \r\n/* foo\r\n bar\r\n */ \nq'
    tokens = iolexer.tokenize(inp)
    assert tokens[0].name != 'Comment'
    assert tokens[1].name != 'Comment'
